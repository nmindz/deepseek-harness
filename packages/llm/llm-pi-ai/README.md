...
- **`claude-opus-5-5` is a provisional catalog entry** — Anthropic serves the id ahead of the pinned pi-ai catalog and publishes it with `claude-opus-5`'s specs, so `catalogModels` copies that installed entry under it. A route profile declares neither `cost` nor a withheld compat field such as `supportsMidConvoEffort`, so inheritance is the only way the model reaches its twin pricing and dispatch behavior. The installed catalog wins once a pi-ai release ships the id; delete the entry then.
